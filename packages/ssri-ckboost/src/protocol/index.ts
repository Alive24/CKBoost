...
export class Protocol extends ssri.Trait {
   ...
   */
  static createProtocolData(
    data: ProtocolDataLike
  ): ReturnType<typeof ProtocolData.decode> {
    ...
    return {
      campaigns_approved: data.campaigns_approved.map((campaign) => ({
        ...
        metadata: {
          funding_info: campaign.metadata.funding_info.map((funding) => ({
            ...
            nft_assets: funding.nft_assets.map((script) =>
          ...
          verification_requirements: Number(
            ccc.numFrom(campaign.metadata.verification_requirements)
          ),
          last_updated: ccc.numFrom(campaign.metadata.last_updated),
          categories: campaign.metadata.categories.map((cat) =>
            ccc.hexFrom(cat)
          ),
        ...
        quests: campaign.quests.map((quest) => ({
          ...
          rewards_on_completion: quest.rewards_on_completion.map((asset) => ({
            ...
            nft_assets: asset.nft_assets.map((script) =>
              ccc.Script.from(script)
            ),
        ...
        endorser_info: {
          endorser_lock_hash: ccc.hexFrom(
            campaign.endorser_info.endorser_lock_hash
          ),
          endorser_name: ccc.hexFrom(campaign.endorser_info.endorser_name),
          endorser_description: ccc.hexFrom(
            campaign.endorser_info.endorser_description
          ),
          website: ccc.hexFrom(campaign.endorser_info.website),
          social_links: campaign.endorser_info.social_links.map((link) =>
            ccc.hexFrom(link)
          ),
      ...
      tipping_proposals: data.tipping_proposals.map((proposal) => ({
        ...
        metadata: {
          ...
          contribution_type_tags: proposal.metadata.contribution_type_tags.map(
            (tag) => ccc.hexFrom(tag)
          ),
          description: ccc.hexFrom(proposal.metadata.description),
          proposal_creation_timestamp: ccc.numFrom(
            proposal.metadata.proposal_creation_timestamp
          ),
        },
        amount: ccc.numFrom(proposal.amount),
        tipping_transaction_hash:
          proposal.tipping_transaction_hash === null
            ? undefined
            : proposal.tipping_transaction_hash
              ? ccc.hexFrom(proposal.tipping_transaction_hash)
              : undefined,
        approval_transaction_hash: proposal.approval_transaction_hash.map(
      ...
      tipping_config: {
        approval_requirement_thresholds:
          data.tipping_config.approval_requirement_thresholds.map((threshold) =>
            ccc.numFrom(threshold)
          ),
        expiration_duration: ccc.numFrom(
          data.tipping_config.expiration_duration
        ),
      },
      ...
      protocol_config: {
        admin_lock_hash_vec: data.protocol_config.admin_lock_hash_vec.map(
          (hash) => ccc.hexFrom(hash)
        ),
        script_code_hashes: {
          ckb_boost_protocol_type_code_hash: ccc.hexFrom(
            data.protocol_config.script_code_hashes
              .ckb_boost_protocol_type_code_hash
          ),
          ckb_boost_protocol_lock_code_hash: ccc.hexFrom(
            data.protocol_config.script_code_hashes
              .ckb_boost_protocol_lock_code_hash
          ),
          ckb_boost_campaign_type_code_hash: ccc.hexFrom(
            data.protocol_config.script_code_hashes
              .ckb_boost_campaign_type_code_hash
          ),
          ckb_boost_campaign_lock_code_hash: ccc.hexFrom(
            data.protocol_config.script_code_hashes
              .ckb_boost_campaign_lock_code_hash
          ),
          ckb_boost_user_type_code_hash: ccc.hexFrom(
            data.protocol_config.script_code_hashes
              .ckb_boost_user_type_code_hash
          ),
          accepted_udt_type_code_hashes:
            data.protocol_config.script_code_hashes.accepted_udt_type_code_hashes.map(
              (hash) => ccc.hexFrom(hash)
            ),
          accepted_dob_type_code_hashes:
            data.protocol_config.script_code_hashes.accepted_dob_type_code_hashes.map(
              (hash) => ccc.hexFrom(hash)
            ),
        },
      },
    ...
    return {
      ...
      script_code_hashes: {
        ...
        ckb_boost_user_type_code_hash: config.script_code_hashes
          ...
          : ccc.hexFrom("0x" + "0".repeat(64)),
        accepted_udt_type_code_hashes: (
          config.script_code_hashes?.accepted_udt_type_code_hashes || []
        ).map((hash) => ccc.hexFrom(hash)),
        accepted_dob_type_code_hashes: (
          config.script_code_hashes?.accepted_dob_type_code_hashes || []
        ).map((hash) => ccc.hexFrom(hash)),
      },
    };
  }
