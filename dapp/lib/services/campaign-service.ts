...
import { CampaignData, type CampaignDataLike, type QuestDataLike } from "ssri-ckboost/types"
import { fetchCampaignCells, fetchCampaignByTypeHash } from '../ckb/campaign-cells'
...
export class CampaignService {
  ...
  constructor(signer: ccc.Signer, campaign: Campaign, executor: ssri.Executor) {
    ...
    const campaignTypeScript = deploymentManager.getContractTypeScript(network, 'ckboostCampaignType')
  ...
  async updateCampaign(
    ...
    try {
      ...
      const currentCampaignData = CampaignData.decode(currentCampaign.outputData)
  ...
  async approveCompletion(
    ...
    try {
      ...
      console.log("Quest completion approved:", { campaignTypeHash, questId, txHash })
      return txHash
    } catch (error) {
   ...
   */
  static async searchCampaigns(query: string, signer?: ccc.Signer): Promise<ccc.Cell[]> {
    const campaigns = await this.getAllCampaigns(signer)
    
    return campaigns.filter(campaign => {
      const campaignData = CampaignData.decode(campaign.outputData)
      return campaignData.metadata.title.toLowerCase().includes(query.toLowerCase()) ||
      campaignData.metadata.short_description.toLowerCase().includes(query.toLowerCase()) ||
      campaignData.metadata.long_description.toLowerCase().includes(query.toLowerCase())
    })
  }
   ...
   */
  static async getCampaignsByCategory(category: string, signer?: ccc.Signer): Promise<ccc.Cell[]> {
    const campaigns = await this.getAllCampaigns(signer)
    
    return campaigns.filter(campaign => {
      const campaignData = CampaignData.decode(campaign.outputData)
      return campaignData.metadata.categories.some(cat => 
